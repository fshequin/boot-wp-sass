...
global $apc_glb;
$apc_glb = array();
$apc_glb['col_black'] =& $col_black;
$apc_glb['time'] =& $time;
$apc_glb['MYREQUEST'] =& $MYREQUEST;
$apc_glb['MY_SELF_WO_SORT'] =& $MY_SELF_WO_SORT;
$apc_glb['MY_SELF'] =& $MY_SELF;
$apc_glb['AUTHENTICATED'] =& $AUTHENTICATED;
...
//
$PHP_SELF = admin_url( 'tools.php?page=wp-apc-panel-page' );
$time = time();
$host = php_uname('n');
if($host) { $host = '('.$host.')'; }
...
function sortheader($key,$name,$extra='') {
    global $apc_glb;
    $MYREQUEST = $apc_glb['MYREQUEST'];
    $MY_SELF_WO_SORT = $apc_glb['MY_SELF_WO_SORT'];
...
function put_login_link($s="Login")
{
    ...
    $MYREQUEST = $apc_glb['MYREQUEST'];
    $MY_SELF = $apc_glb['MY_SELF'];
    $AUTHENTICATED = $apc_glb['AUTHENTICATED'];
...
div.content { padding:1em 1em 1em 1em; position:relative; width:97%; z-index:100; }
...
ol.menu li { display:inline; margin-right:0.7em; list-style:none; font-size:85%; position: static;}
ol.menu a {
...
<div class="head">
    ...
    <span id="make_visible">
        <input type="checkbox" id="makevisible" disabled name="make_visible"<?= get_option('wap_visible_to_site_admins') == 1 ? ' checked' : '' ?> />
        <?php wp_nonce_field( 'wap_visible_to_site_admins', 'wap_visible_to_site_admins_nonce' ); ?>
        <label for="makevisible"> Make this page visible to site admin</label>
    </span>
    <?php endif; ?>
...
EOB;
	...
	echo <<<EOB
		...
		<input type=hidden name=page value=wp-apc-panel-page>
		<select name=SCOPE>
EOB;
	echo 
